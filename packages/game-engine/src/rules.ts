import { Player, RoomSettings, GAME_LIMITS, getMaxImpostorsForPlayerCount } from "@impostor/shared";
...
export function validateCanStartGame(
  players: Player[],
  ...
  const invalidCategories = settings.selectedCategories.filter(
...
export function checkWinConditions(
  ...
  for (const player of players.values()) {
    if (player.isSpectator || player.isEliminated) continue;
...
export function countVotes(
  ...
  for (const targetId of votes.values()) {
    if (candidateFilter && !candidateFilter.includes(targetId)) continue;
