import { SCORING_RULES } from "@impostor/shared";
...
export function updateScoresForRound(currentScores, players, impostorIds, votes, eliminatedId, winner, lastChanceBonusPlayerId = null) {
    ...
    if (winner) {
        for (const player of players.values()) {
            if (player.isSpectator || !newScores[player.id])
                continue;
            const isImpostor = impostorIds.has(player.id);
