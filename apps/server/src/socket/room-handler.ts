...
export function registerRoomHandlers(io: TypedServer, socket: TypedSocket) {
  ...
  socket.on("room:create", (rawPayload, callback) => {
    try {
      ...
      const room = globalRoomStore.createRoom(
        hostPlayer,
        {
          mode: input.mode,
  ...
  socket.on("room:reconnect", (payload, callback) => {
    try {
      ...
      const isValid = globalRoomStore.verifySessionToken(
  ...
  socket.on("room:kick", ({ targetPlayerId }, callback) => {
    try {
      ...
      if (!target) return callback({ success: false, error: "Jugador no encontrado" });
